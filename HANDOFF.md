...
- スマホ：表を隠し、日ごとのカードを表示
- 上部ジャンプ：「上部」「今日」「第1週〜第5週」「献立登録」
- 右下の固定ボタン：「今日」アイコンと `↑`（PC・スマホ共通）
- 「今日」は日本時間の現在日をページ表示時に判定し、PCでは表の該当行、スマホでは日別カードへ移動
- 第1週〜第5週は、PCでは各週先頭の表の行、スマホでは各週先頭の日別カードへ移動
- ブラウザタブ用アイコンは `index.html` 内にSVGデータとして埋め込み
...
- Pythonテスト：11件すべて成功
- 公開URLでタイトル・セブン候補・A/B比較・副菜・日別理由を確認
- スマホ幅390pxで日別カード22件を確認
- 横方向のレイアウト崩れなし
- 「今日」の移動先がPCの表とスマホの日別カードで自動的に切り替わる構成
