...
固定ページ上部のジャンプメニューから、今日、各週、献立登録へ直接移動できます。今日と第1週〜第5週は、PCでは表の該当行、スマホでは該当する日別カードへ移動します。右下には「今日」アイコンと「↑」が常に表示されます。今日の日付は日本時間でページを開くたびに判定されます。
...
このURLをPCとスマホのブラウザでブックマークしてください。PCでは一覧表、スマホでは日ごとのカード表示になります。上部メニューと右下のアイコンから目的の場所へ移動できます。
